...
class NSSerialPort extends SerialPort {
    ...
    public synchronized void addEventListener(final SerialPortEventListener lstnr) throws TooManyListenersException {
        ...
        if ((this.notifyOnDSRFlag || this.notifyOnRIFlag || this.notifyOnCDFlag || this.notifyOnORFlag
                || this.notifyOnPEFlag || this.notifyOnFEFlag || this.notifyOnCTSFlag || this.notifyOnBIFlag)
    ...
    public synchronized void notifyOnBreakInterrupt(final boolean notify) {
        ...
        if (notify && !this.notifyOnBIFlag) {
            ...
            // check all other related flags, all must be false
            if (!this.notifyOnCTSFlag && !this.notifyOnDSRFlag && !this.notifyOnRIFlag && !this.notifyOnCDFlag
                    && !this.notifyOnORFlag && !this.notifyOnPEFlag && !this.notifyOnFEFlag) {
                if (this.statusThread != null) {
                    this.statusThread.setStopThreadFlag(1);
                    this.statusThread = null;
                }
    ...
    public synchronized void notifyOnCTS(final boolean notify) {
        ...
        if (notify && !this.notifyOnCTSFlag) {
            ...
            // check all other related flags, all must be false
            if (!this.notifyOnDSRFlag && !this.notifyOnRIFlag && !this.notifyOnCDFlag && !this.notifyOnORFlag
                    && !this.notifyOnPEFlag && !this.notifyOnFEFlag && !this.notifyOnBIFlag) {
                if (this.statusThread != null) {
                    this.statusThread.setStopThreadFlag(1);
                    this.statusThread = null;
                }
    ...
    public synchronized void notifyOnCarrierDetect(final boolean notify) {
        ...
        if (notify && !this.notifyOnCDFlag) {
            ...
            // check all other related flags, all must be false
            if (!this.notifyOnCTSFlag && !this.notifyOnDSRFlag && !this.notifyOnORFlag && !this.notifyOnRIFlag
                    && !this.notifyOnPEFlag && !this.notifyOnFEFlag && !this.notifyOnBIFlag) {
                if (this.statusThread != null) {
                    this.statusThread.setStopThreadFlag(1);
                    this.statusThread = null;
    ...
    public synchronized void notifyOnDSR(final boolean notify) {
        ...
        if (notify && !this.notifyOnDSRFlag) {
            ...
            // check all other related flags, all must be false
            if (!this.notifyOnCTSFlag && !this.notifyOnRIFlag && !this.notifyOnCDFlag && !this.notifyOnORFlag
                    && !this.notifyOnPEFlag && !this.notifyOnFEFlag && !this.notifyOnBIFlag) {
                if (this.statusThread != null) {
                    this.statusThread.setStopThreadFlag(1);
                    this.statusThread = null;
    ...
    public synchronized void notifyOnFramingError(final boolean notify) {
        ...
        if (notify && !this.notifyOnFEFlag) {
            ...
            // check all other related flags, all must be false
            if (!this.notifyOnCTSFlag && !this.notifyOnDSRFlag && !this.notifyOnRIFlag && !this.notifyOnCDFlag
                    && !this.notifyOnORFlag && !this.notifyOnPEFlag && !this.notifyOnBIFlag) {
                if (this.statusThread != null) {
                    this.statusThread.setStopThreadFlag(1);
                    this.statusThread = null;
                }
    ...
    public synchronized void notifyOnParityError(final boolean notify) {
        ...
        if (notify && !this.notifyOnPEFlag) {
            ...
            // check all other related flags, all must be false
            if (!this.notifyOnCTSFlag && !this.notifyOnDSRFlag && !this.notifyOnRIFlag && !this.notifyOnCDFlag
                    && !this.notifyOnORFlag && !this.notifyOnFEFlag && !this.notifyOnBIFlag) {
                if (this.statusThread != null) {
                    this.statusThread.setStopThreadFlag(1);
                    this.statusThread = null;
                }
    ...
    public synchronized void notifyOnRingIndicator(final boolean notify) {
        ...
        if (notify && !this.notifyOnRIFlag) {
            ...
            // check all other related flags, all must be false
            if (!this.notifyOnCTSFlag && !this.notifyOnDSRFlag && !this.notifyOnCDFlag && !this.notifyOnORFlag
                    && !this.notifyOnPEFlag && !this.notifyOnFEFlag && !this.notifyOnBIFlag) {
                if (this.statusThread != null) {
                    this.statusThread.setStopThreadFlag(1);
                    this.statusThread = null;
    ...
    public void setFlowControlMode(final int flowctrl) throws UnsupportedCommOperationException {
        ...
        if ((this.fd == -1) ||
                 ...
                 **************************/
        (((flowctrl & FLOWCONTROL_RTSCTS_IN) != 0) && ((flowctrl & FLOWCONTROL_XONXOFF_OUT) != 0))
                || (((flowctrl & FLOWCONTROL_XONXOFF_IN) != 0) && ((flowctrl & FLOWCONTROL_RTSCTS_OUT) != 0))
                || (((flowctrl & FLOWCONTROL_RTSCTS_IN) != 0) && ((flowctrl & FLOWCONTROL_XONXOFF_IN) != 0))
                || (((flowctrl & FLOWCONTROL_RTSCTS_OUT) != 0) && ((flowctrl & FLOWCONTROL_XONXOFF_OUT) != 0))) {
            throw new UnsupportedCommOperationException();
        }
    ...
    public void setSerialPortParams(final int bd, final int db, final int sb, final int par)
        ...
        if ((par != PARITY_NONE) && (par != PARITY_ODD) && (par != PARITY_EVEN) && (par != PARITY_MARK)
                && (par != PARITY_SPACE)) {
            throw new UnsupportedCommOperationException();
        }
